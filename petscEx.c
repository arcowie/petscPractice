...
PetscErrorCode formRHS(DM da, Vec b){
    ...
    for (j = info.ys; j < info.ys+info.ym; j++){
        ...
        for(i = info.xs; i < info.xs+info.xm; i++){
            ...
            {
                ab[j][i]= 0.0; //on boundry: 1*u=0
            }
            else{
                f = 2.0*((1-6.0*x*x))*y*y*(1.0-y*y)+(1.0-6.0*y*y)*x*x*(1.0-x*x);
                ab[j][i] = hx*hy*f;
            }
...
int main(int argc, char *argv[]) {
    ...
    // create RHS, b, approx sol u, exact sol uexact
    ...
    PetscCall(DMDAGetLocalInfo(da, &info));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "on %d x %d grid: error |u-uexact|_2 = %g\n",info.mx, info.my, errnorm));
